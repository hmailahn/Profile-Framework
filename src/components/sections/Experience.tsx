...
export default function Experience() {
    ...
    nodes.push(
      {
        ...
        mainItem: {
          ...
          description:
            "Continuing to build scalable solutions and mentor others.",
      ...
      },
      {
        ...
        mainItem: {
          ...
          description:
            "Driving technical strategy and cross-team architecture.",
  ...
  const generatePath = () => {
    ...
    path =
      `M ${first.x - 300} 0 C ${first.x - 150} 0, ${first.x - 150} ${first.y}, ${first.x} ${first.y}` +
      path.substring(path.indexOf("C"));
  ...
  const x = useTransform(
    ...
    [
      0,
      -TOTAL_WIDTH + (typeof window !== "undefined" ? window.innerWidth : 1000),
    ],
  );
                    ...
                    {isMain ? (
                      <>
                        <div className="flex items-center gap-3 mb-3">
                          {node.mainItem?.logo ? (
                            <img
                              src={node.mainItem.logo}
                              alt={node.title}
                              className="w-16 h-10 rounded-md object-contain bg-white p-1"
                            />
                          ) : (
                            <div className="w-10 h-10 rounded-lg bg-primary/10 flex items-center justify-center text-primary">
                              {node.mainItem?.type === "work" ? (
                                <FiBriefcase size={20} />
                              ) : (
                                <FiBook size={20} />
                              )}
      ...
      <div className="sticky top-0 flex h-screen items-center overflow-hidden">
        <motion.div className="w-full h-full flex items-center">
          ...
          >
            {/* Winding Path SVG */}
            <div
              className="absolute top-1/2 left-0 h-0 overflow-visible -z-10"
            ...
            <div className="relative h-full">
              ...
              `}
                    ...
                    {isMain && node.date && (
                      <div
                        className={`absolute left-1/2 -translate-x-1/2 text-sm font-bold text-primary/80 whitespace-nowrap
